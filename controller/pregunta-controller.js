...
const createPreguntaFromBody = (id, { preguntaTexto, opcion1, opcion2, opcion3, opcion4, respuestaCorrecta }) => {
    return new Pregunta(id, preguntaTexto, opcion1, opcion2, opcion3, opcion4, respuestaCorrecta);
};
...
router.post('/', async (req, res) => {
    ...
    handlePreguntaResponse(res, preguntaServiceInstance.crearPregunta(nuevaPregunta), true, 'No se pudo crear la pregunta.');
});
...
router.put('/:id', async (req, res) => {
    ...
    handlePreguntaResponse(res, preguntaServiceInstance.actualizarPregunta(preguntaModificada), false, `Pregunta con ID ${preguntaId} no encontrada.`);
});

router.delete('/:id', async (req, res) => {
    const preguntaId = parseInt(req.params.id, 10);
    handlePreguntaResponse(res, preguntaServiceInstance.eliminarPregunta(preguntaId), false, `No se encontró la pregunta con ID ${preguntaId}.`);
});

router.get('/azar', async (req, res) => {
    handlePreguntaResponse(res, preguntaServiceInstance.obtenerPreguntaAzar(), false, 'Hubo un problema al obtener la pregunta al azar.');
});

router.get('/', async (req, res) => {
    const { palabraClave, orden } = req.query;
    handlePreguntaResponse(res, preguntaServiceInstance.obtenerTodasLasPreguntas(palabraClave, orden), false, 'No se pudieron obtener las preguntas.');
});
